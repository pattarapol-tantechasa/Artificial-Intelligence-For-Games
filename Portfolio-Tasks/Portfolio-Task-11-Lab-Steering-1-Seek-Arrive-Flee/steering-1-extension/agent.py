...
class Agent(object):
    ...
    def __init__(self, world=None, scale=30.0, mass=1.0, mode='seek'):
        ...
        # non-uniform acceleration limits per direction
        self.max_force_forward = 500.0
        self.max_force_side = 250.0
        self.max_force_reverse = 100.0
    ...
    def update(self, delta):
        ...
        # decompose acceleration into forward/side components
        fwd_amount = acceleration.dot(self.heading)
        fwd_vec = self.heading * fwd_amount
        side_vec = acceleration - fwd_vec
        # clamp each component separately
        if fwd_amount >= 0:
            if fwd_vec.length() > self.max_force_forward:
                fwd_vec = self.heading * self.max_force_forward
        else:
            if fwd_vec.length() > self.max_force_reverse:
                fwd_vec = self.heading * -self.max_force_reverse
        if side_vec.length() > self.max_force_side:
            side_vec = side_vec.get_normalised() * self.max_force_side
        acceleration = fwd_vec + side_vec
        # new velocity
        self.vel += acceleration * delta
        # apply friction to gradually slow agent when no force is applied

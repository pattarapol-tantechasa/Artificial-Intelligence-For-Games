...
from states import PatrolState, DETECTION_RANGE
...
class Agent(object):
    ...
    def __init__(self, world=None, scale=30.0, mass=1.0, mode='seek'):
        ...
        # Limits for physics stability
        self.max_speed = 8.0 * scale
        self.max_force = 100.0 * scale

        # FSM: start in patrol mode
        self.current_state = None
        self.change_state(PatrolState())

    def calculate(self, delta):
        """Delegates steering calculation to the active FSM state."""
        if self.current_state:
            force = self.current_state.execute(self, delta)
        else:
            force = Vector2D()
        self.force = force
        return force
    ...
    def detect_enemy(self):
        """Returns True if any live enemy is within detection range."""
        for enemy in getattr(self.world, 'enemies', []):
            if enemy.alive and self.pos.distance(enemy.pos) < DETECTION_RANGE:
                return True
        return False
    ...
    def _nearest_enemy(self):
        ...
        for enemy in getattr(self.world, 'enemies', []):
            if enemy.alive:
                ...
                if d < min_dist:
                    min_dist, nearest = d, enemy
        return nearest

    def shoot(self):
        target = self._nearest_enemy()
        if target:
            self.world.projectiles.append(Projectile(self.pos.copy(), target))
    ...
    def pursuit(self, evader):
        ...
        ### STUDENT TODO: Implement pursuit logic
        return Vector2D()

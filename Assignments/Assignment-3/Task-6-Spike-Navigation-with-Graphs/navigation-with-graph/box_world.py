...
box_types = {
	"CLEAR":{"symbol":'.', "cost":{"CLEAR":1, "MUD":2, "WATER":5, "FOREST":2, "HILL":3, "ICE":6}, "colour":"WHITE"},
	"MUD":{"symbol":'m', "cost":{"CLEAR":2, "MUD":4, "WATER":9, "FOREST":3, "HILL":5, "ICE":8}, "colour":"BROWN"},
	"WATER":{"symbol":'~', "cost":{"CLEAR":5, "MUD":9, "WATER":10, "FOREST":7, "HILL":8, "ICE":8}, "colour":"AQUA"},
	"FOREST":{"symbol":'f', "cost":{"CLEAR":2, "MUD":3, "WATER":8, "FOREST":3, "HILL":5, "ICE":6}, "colour":"DARK_GREEN"},
	"HILL":{"symbol":'h', "cost":{"CLEAR":2, "MUD":4, "WATER":10, "FOREST":4, "HILL":7, "ICE":9}, "colour":"ORANGE"},
	"ICE":{"symbol":'i', "cost":{"CLEAR":2, "MUD":4, "WATER":7, "FOREST":5, "HILL":8, "ICE":11}, "colour":"BLUE"},
...
class BoxWorld(object):
	...
	def _add_edge(self, from_idx, to_idx, distance=1.0):
		...
		if "cost" in box_types[b[from_idx].type] and b[to_idx].type in box_types[b[from_idx].type]["cost"]:
			...
			threat = getattr(b[to_idx], 'threat', 0)
			self.graph.add_edge(Edge(from_idx, to_idx, (cost + threat) * distance))
	
	def _add_sky_edge(self, from_idx, to_idx, distance=1.0):
		b = self.boxes
		if "cost" in box_types[b[from_idx].type] and "cost" in box_types[b[to_idx].type]:
			self.sky_graph.add_edge(Edge(from_idx, to_idx, 1 * distance))
	...
	def reset_navgraph(self):
		...
		self.sky_graph.cost_h = self._hypot
		...
		for i, box in enumerate(self.boxes):
			...
			self.sky_graph.add_node(Node(idx=i))
		...
		for i, box in enumerate(self.boxes):
			...
			if (j-1) < len(self.boxes) and (j%nx - 1) >= 0:
				...
				self._add_sky_edge(i, j-1, 1.4142)
			# UP RIGHT (i + nx + 1)
			j = i + nx
	...
	def plan_path(self, search, limit, graph, start_idx, target_idx):
		...
		'''
		cls = SEARCHES[search]
		return cls(graph, start_idx, target_idx, limit)

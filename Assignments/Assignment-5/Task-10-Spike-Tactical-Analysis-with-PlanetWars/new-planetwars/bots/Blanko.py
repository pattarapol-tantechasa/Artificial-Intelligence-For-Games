...
controller instance. The Player will provide itself (acting as a ``GameInfo``
façade) to your bot ``update`` method each time it is called.

The ``gameinfo`` instance is a façade of the state of the game for the Player,
and includes all planets and fleets that currently exist. Note that the
details are limited by "fog of war" vision and only match what you can see.
If you want to know more you'll need to scout!

A gameinfo instance has various (possibly useful) dict's for you to use::

    # all planets and fleets (that you own or can see)
    planets
    fleets
    ...
    # dict's of just the enemy planets/fleets (fog limited)
    enemy_planets
    enemy_fleets
